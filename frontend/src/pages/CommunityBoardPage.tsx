  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-6">
          ...
          <button
            ...
            className={`shrink-0 rounded-full px-4 py-1 text-sm font-medium transition-colors ${
              activeCategory === cat
                ...
                : "border border-gray-300 text-gray-600 hover:border-indigo-500 hover:text-indigo-600"
            }`}
          >
            {cat}
          </button>
      ...
      {!loading && posts.length > 0 && (
        <ul className="flex flex-col gap-4">
          {posts.map((post: any) => (
            <li key={post.id} className="rounded-lg border border-gray-200 overflow-hidden">
              <PostCard
                post={{
                  id: post.id,
                  title: post.title,
